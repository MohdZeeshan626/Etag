...
class MarshMallowPermissions(private val mFragment: Fragment) {
    ...
    val isRecordAudioPermission: Boolean
        get() = ContextCompat.checkSelfPermission(
            mActivity,
            Manifest.permission.RECORD_AUDIO
        ) == PackageManager.PERMISSION_GRANTED

    val checkPermissionsForCameraCapture: Array<String>
        get() = mCameraCapturePermissions
            .filter {
                ContextCompat.checkSelfPermission(mActivity, it) !=
                        PackageManager.PERMISSION_GRANTED
            }
            .toTypedArray()
    ...
    fun reqPermissionsForCameraCapture(selfie: Boolean=false) {
