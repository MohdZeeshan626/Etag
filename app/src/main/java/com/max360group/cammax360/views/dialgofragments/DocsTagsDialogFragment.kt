...
class DocsTagsDialogFragment : BaseDialogFragment(), View.OnClickListener {
    companion object {
        ...
        fun newInstance(
            ...
            mPosition: Int
        ): DocsTagsDialogFragment {
            var mFragment = DocsTagsDialogFragment()
            var mBundle = Bundle()
            mBundle.putInt(PARAM_POSITION, mPosition)
            mBundle.putParcelableArrayList(PARAM_VIDEO_LIST, mDocsList)
    ...
    private var mDocsList = ArrayList<DocsModel>()
    ...
    override fun init() {
        ...
        arguments?.let {
            mDocsList =
                it.getParcelableArrayList<DocsModel>(PARAM_VIDEO_LIST) as ArrayList<DocsModel>
            mPosition = it.getInt(PARAM_POSITION)
        }

        //Set data default data
        etMediaName.setText(mDocsList[mPosition].docsName)
        mTagsList.addAll(mDocsList[mPosition].tags!!)
        ...
        when (p0?.id) {
            R.id.btnSubmit -> {
                if (mTagsList.isEmpty()) {
                    ...
                    mDocsList[mPosition].tags=mTagsList
                    mDocsList[mPosition].docsName=etMediaName.text.toString().trim()
