...
class DocumentsAdapter(var mFragment: Fragment) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    fun updateAdapter(mJobMediaList: ArrayList<JobMediaList>?, rvList: RecyclerView, mType: Int) {
        //Set adapter
        var mDocumentsItemsAdapter=DocumentsItemsAdapter(mFragment,mJobMediaList)
        var manager = GridLayoutManager(mFragment.requireContext(), this.mType)
        rvList.layoutManager = manager
        rvList.adapter = mDocumentsItemsAdapter
        mDocumentsItemsAdapter.updateData(mType)
    }
